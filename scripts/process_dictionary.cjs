...
const raw1 = require('../src/data/raw1.cjs');
const raw2 = require('../src/data/raw2.cjs');
...
const cleanWords = combined
    .map(w => w.replace(/^"|"$/g, '').trim()) // Remove extra quotes
    .filter(w => {
        if (!w) return false;
...
// Generate file content (ESM format for the React app)
